...
### Milestone 16: Parallel Programme Execution (V3) (In Progress)
...
**Milestone 16 (Parallel Programme Execution) is In Progress. Milestone 15 is complete — V2 is done.** Milestones M1–M9 and M11–M15 are complete. **M15 shipped in v0.50.0** over Sprints 58–60: host-enforced **budgets** on the axes that can genuinely be enforced (wall-clock, attempts, review-cycles, concurrency) with turn/token/cost honestly disclaimed as policy-only; **typed rejection**; a **plane-owned integration target**, keyed by canonical repository path, that an agent cannot influence by any git operation — closing the acceptance-oracle laundering class structurally rather than by patching an ancestry check; the **race-safe integration transaction** (serialize → rebase → re-verify the *merged* result → compare-and-swap); the human `verified → approval_required → approved → integrated` gate with CLI, Web and TUI surfaces; and the **Guild Master finally acting** — `guild-control-mcp` over a **separate agent socket**, with consequential operations reduced to **human-confirmed proposals**. That last piece is the §6 lethal-trifecta defence made structural: the Guild Master reads untrusted project docs, so a poisoned README may *propose*, never *execute*. (M14 shipped in **v0.49.0** — independent, digest-pinned artifact verification with a test-integrity gate and null-agent floor; M13 in **v0.48.0** — the control-plane V1 foundation.) The wider **"controlling Guild Master" control-plane arc** is M13–M17 (design and the graded response to two evaluation rounds in `docs/guild-master-plan.md`; evidence in `docs/guild-master-control.md`): **V1** = M13 + M14, **V2** = M15, both now complete; **V3** = M16 parallel execution and M17 typed steering (demoted). **M16 (Parallel Programme Execution) is now In Progress** — the merge queue built in M15 is currently insurance rather than load-bearing, because one active Task per project means two integrations cannot be in flight; M16 is what makes it earn its keep. Also Planned: M10 (Homebrew).
