...
**One M20 bullet was not delivered, and saying so is cheaper than letting it be found.** The milestone's part-one list promised that *the Guild Master can propose a programme from what it sees across projects*. It cannot: the operations are tiered (`form_programme` = proposal), but `guild-control-mcp` exposes no programme tool, so the Guild Master cannot even list them, and `resolveProposal` has no case for them, so a confirmed proposal would fail closed. Tiering reserves authority over something that does not exist yet — the recurring defect this repository keeps catching in itself, written this time into a milestone's own description. Filed as **#82**.
