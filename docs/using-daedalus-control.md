...
for a readable SENTENCE as well as a short one — the prompt carries a real
finding that used only everyday words and was still unreadable ("a prose turn
whose run then failed…"), with the three faults named and the plain rewrite
beside it. Word choice was never the problem; stacked nouns, a verb on the wrong
subject and a missing actor were. The reviewer may use the project's real
identifiers and may not coin compound terms of its own — it has just spent an hour reading one
...
missing.** It cannot push, clone a private repository, or create one. A container
with the network, an objective the plane treats as untrusted, and a push-capable
key is a bad trade — and ssh cannot scope a key to one repository, so the grant
would always be *"write anywhere this key reaches, for the whole run"*.

The Job is **told this up front**, and told to stop and say so if its objective
needs it. That matters because of how it failed before the note existed: a
repository-split Task spent its entire attempt discovering there were no
credentials and then delivered a patch file and a handoff document. A plan for a
human is not a smaller version of the work — it is a different deliverable, and
it looks like a result until somebody reads it.

**So write objectives that end where the Job's reach ends.** "Prepare the split on
a branch" is a Task; "split the repo and push it to a new remote" is not. The push
is yours.
