...
| 49 | Split `WebServer` god-object — `internal/web/web.go` (~1200 lines) holds 31 methods spanning 6 domains (project lifecycle, dashboard, roadmap/backlog/sprints, foreman, programmes, terminal/WebSocket relay). Violates SRP per CONTRIBUTING.md. First pass: move handlers into topic files (`projects.go`, `roadmap.go`, `foreman.go`, `programmes.go`, `terminal.go`, `state.go`) within the `web` package, keeping `WebServer` as the shared-deps holder. Second pass (optional): per-domain handler structs with only the deps each needs |
| 50 | Thin `cmd/daedalus/main.go` dispatcher — `main.go` (~1675 lines) inlines all 13 subcommand handlers plus git-clone, project-resolution, and Docker orchestration helpers. Move each subcommand into its own file (`foreman.go`, `skills.go`, `personas.go`, `programmes.go`, `clone.go`, ...) so `main.go` stays a thin dispatcher. Same SRP rationale as #49 |
