...
_daedalus() {
    ...
    case "${words[1]}" in
        ...
        version)
            ...
            ;;
        task)
            COMPREPLY=($(compgen -W "create list status dispatch verify review approve reject integrate approvals proposals depends target retry replan events cancel --project --objective --acceptance --wall-clock --max-attempts --max-review-cycles --concurrency --rebase --note --sync" -- "${cur}"))
            ;;
        *)
        ...
        args)
            case ${words[1]} in
                ...
                version)
                    ...
                    ;;
                task)
                    _values 'action' create list status dispatch verify review approve reject integrate approvals proposals depends target retry replan events cancel --project --objective --acceptance --wall-clock --max-attempts --max-review-cycles --concurrency --rebase --note --sync
                    ;;
                *)
                    _describe -t flags 'flag' flags
                    ;;
...
complete -c daedalus -n '__fish_seen_subcommand_from task' -a 'create list status dispatch verify review approve reject integrate approvals proposals depends target retry replan events cancel' -d 'task action'
...
complete -c daedalus -n '__fish_seen_subcommand_from task' -l on -d 'Task id this task depends on' -r 
