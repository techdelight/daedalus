...
| 5 | Flip the runner path to default (drop `DAEDALUS_USE_RUNNER=1`) and retire the tmux launch path — **gate cleared** by item 4's parity pass (2026-07-27); ready to flip. Enabler already landed: the Web dashboard is a first-class runner client (`DAEDALUS_USE_RUNNER=1 daedalus web` → "Open" launches via the coordinator). The default-flip + tmux retirement is drafted on `feat/runner-default-flip`. | In Progress |
