...
### Sprint 45: Project-Management MCP Tools & File-Derived State
...
| 4 | **Lifecycle MCP tools (`cmd/project-mgmt-mcp`)** — `add/remove/move/start/finish/pause_milestone` + `…_sprint`, each reading `ROADMAP.md`/`SPRINTS.md`, applying the #2 mutation, writing back, returning new state or a structured error. Tests; register in the server (+ `internal/mcpclient` if consumed) | |
| 5 | **File-derived read state (#52)** — derive vision (`VISION.md`), version (`VERSION`), progress (sprint item statuses → %) host-side; remove the self-report write tools (`report_progress`/`set_vision`/`set_version`). Point host readers at the derived source. Tests | |
| 6 | **Agent guidance + docs + close** — seed guidance so the agent manages the roadmap via these tools; document tools + `Paused` in `docs/structured-docs.md`; CHANGELOG; dogfood; close M7 + release | |
