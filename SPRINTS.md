...
| 5 | Flip the runner path to default (drop `DAEDALUS_USE_RUNNER=1`) and retire the tmux launch path once parity is reached — gated on item 4's real-Docker pass. **Enabler landed:** the Web dashboard is now a first-class runner client (`DAEDALUS_USE_RUNNER=1 daedalus web` → "Open" launches via the coordinator), so the runner path is reachable from CLI + Web; the actual default-flip + tmux retirement still await the parity pass | In progress |
