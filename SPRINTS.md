...
Items 4 (mobile #29) and 7 (retire tmux) are done. Items 2 (runtime mounts), 3 (trust idempotency), and 5 (installer pins) are in progress — the code has landed and is tested/statically sound (item 2's top permission risk is now instrumented with a build-uid record + coordinator preflight warning); only the on-host build/run confirmation remains. Item 1 is Pending and 6 is Deferred: all host-side (real Docker); the full runbook is `docs/m5-verification.md`.
