...
_No active sprint, and no active milestone. Milestone 19 (Verification Independence) closed with Sprint 65; its work is unreleased and staged for v0.54.0, so `master` still sits at v0.52.0 with v0.53.0 prepped but unpublished — the release decision is outstanding, not forgotten. The next focus is undecided. The standing candidates are BACKLOG #69 (real execution termination) and #70 (a durable scheduler), both milestone-sized and both named by the post-arc review as the gap between a serious control plane and the execution substrate underneath it; #80 is smaller and came out of Sprint 65 — whether a non-terminal `failed` should exist at all, or whether every execution outcome should feed the retry ladder._
