...
Items 4 (mobile #29) and 7 (retire tmux) are done. Items 3 (trust idempotency) and 5 (installer pins) are in progress — the code has landed and is tested/statically sound; only the on-host build/run confirmation remains. Items 1, 2 are Pending and 6 is Deferred: all host-side (real Docker); the full runbook is `docs/m5-verification.md`.
